...
<head>
    ...
    <style>
        ...
        .reply-form {
            background: linear-gradient(135deg, rgba(217, 119, 6, 0.05) 0%, rgba(180, 83, 9, 0.02) 100%);
                ...
                <div class="grid md:grid-cols-2 gap-8 items-center">
                    <div class="text-white space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 pt-4">
                            <a href="{{ route('themes.preview', $theme) }}" target="_blank" class="btn-secondary px-8 py-3 rounded-lg font-semibold text-center inline-flex items-center justify-center gap-2 bg-white/10 backdrop-blur text-white border-white/30 hover:bg-white hover:text-gray-900">
                    ...
                    @if($theme->thumbnail_path)
                        <div class="hidden md:block animated-float">
                            <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 shadow-2xl">
                                <img src="{{ asset('storage/' . $theme->thumbnail_path) }}" alt="{{ $theme->name }}" class="w-full rounded-lg shadow-lg">
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <!-- UPDATED: User Reviews Section with Replies -->
                        <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
                            <div class="flex items-center gap-3 mb-6">
                            ...
                            @auth
                                    <form action="{{ route('themes.comment.store', $theme) }}" method="POST" class="mb-8 p-6 bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl border border-orange-100">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <textarea name="comment" rows="4" required class="w-full px-4 py-3 bg-white border-2 border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition" placeholder="Share your experience with this theme..."></textarea>
                                            @error('comment')
                            ...
                            <div class="space-y-6">
                                @forelse($theme->approvedComments as $comment)
                                    <div class="border-b border-gray-200 pb-6 last:border-0">
                                        ...
                                        <div class="hover:bg-gray-50 p-4 rounded-lg transition">
                                            <div class="flex items-start gap-4">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <div class="flex items-center gap-4 comment-actions">
                                                        @auth
                                                            <button onclick="toggleReplyForm({{ $comment->id }})" class="text-orange-600 hover:text-orange-700 text-sm font-semibold flex items-center gap-1">
                                                    ...
                                                    <!-- Reply Form -->
                                                    @auth
                                                        <form id="replyForm{{ $comment->id }}" action="{{ route('themes.comment.store', $theme) }}" method="POST" class="reply-form hidden mt-3">
                                                            @csrf
                                                            <input type="hidden" name="parent_id" value="{{ $comment->id }}">
                                                            <textarea name="comment" rows="3" required class="w-full px-4 py-3 bg-white border-2 border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition text-sm" placeholder="Write your reply..."></textarea>
                                                            <div class="flex gap-2 mt-2">
                                                                <button type="submit" class="px-4 py-2 bg-orange-500 text-white rounded-lg text-sm font-semibold hover:bg-orange-600 transition">
                                                                    <i class="fas fa-paper-plane mr-1"></i> Send Reply
                                                                </button>
                                                                <button type="button" onclick="toggleReplyForm({{ $comment->id }})" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-300 transition">
                                                                    Cancel
                                                                </button>
                                                            </div>
                                                        </form>
                                        ...
                                        <!-- Replies -->
                                        @if($comment->replies->count() > 0)
                                            <div class="reply-indent mt-4 space-y-4">
                                                @foreach($comment->replies as $reply)
                                                    <div class="flex items-start gap-3 p-3 bg-orange-50 rounded-lg hover:bg-orange-100 transition">
                                                        <div class="w-10 h-10 bg-gradient-to-br from-amber-500 to-orange-600 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                                            {{ strtoupper(substr($reply->user->name, 0, 1)) }}
                                                        </div>
                                                        <div class="flex-1">
                                                            <div class="flex items-center justify-between mb-1">
                                                                <div>
                                                                    <p class="font-semibold text-gray-900 text-sm">{{ $reply->user->full_name ?? $reply->user->name }}</p>
    ...
    @if(session('success'))
        <div class="fixed top-4 right-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-4 rounded-xl shadow-2xl z-50 max-w-md animate-slide-in" id="successMessage">
            <div class="flex items-start gap-3">
                ...
                <div><p class="font-bold">Success!</p><p class="text-sm text-white/90">{{ session('success') }}</p></div>
            </div>
        </div>
    @endif
    @if(session('error'))
        <div class="fixed top-4 right-4 bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-4 rounded-xl shadow-2xl z-50 max-w-md animate-slide-in" id="errorMessage">
            <div class="flex items-start gap-3">
                ...
                <div><p class="font-bold">Error!</p><p class="text-sm text-white/90">{{ session('error') }}</p></div>
            </div>
        </div>
    @endif
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all scale-95 opacity-0" id="modalContent">
            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeActivationModal()" class="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-semibold transition">Cancel</button>
