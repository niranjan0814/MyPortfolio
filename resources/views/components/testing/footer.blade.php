...
<footer class="relative overflow-hidden"
    ...
    <div class="absolute inset-0 opacity-10 normal-theme-only">
        <div
            class="absolute top-0 left-0 w-96 h-96 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl animate-blob">
        </div>
        <div
            class="absolute bottom-0 right-0 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000">
        </div>
    </div>
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        @foreach(['About Me|#about', 'Projects|#projects', 'Skills|#skills', 'Contact|#contact'] as $link)
                            ...
                            <li>
                                <a href="{{ $url }}"
                                    class="transition-colors flex items-center gap-2 group hover:opacity-80"
                                    style="color: var(--text-secondary);">
                                    <svg class="w-3 h-3 text-xs group-hover:translate-x-1 transition-transform" fill="none"
                        ...
                        <ul class="space-y-4">
                            ...
                            @if($user->phone)
                                <li>
                                    <a href="tel:{{ $user->phone }}"
                                        class="transition-colors flex items-start gap-3 group hover:opacity-80"
                                        style="color: var(--text-secondary);">
                                        <svg class="w-4 h-4 mt-1" style="color: var(--accent-blue);" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path
                            ...
                            @if($user->email)
                                <li>
                                    <a href="mailto:{{ $user->email }}"
                                        class="transition-colors flex items-start gap-3 group hover:opacity-80"
                                        style="color: var(--text-secondary);">
                                        <svg class="w-4 h-4 mt-1" style="color: var(--accent-purple);" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if($user->address)
                                <li class="flex items-start gap-3" style="color: var(--text-secondary);">
                                    <svg class="w-4 h-4 mt-1" style="color: #ef4444;" fill="currentColor"
                                        viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                            d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z"
                                            clip-rule="evenodd" />
                        ...
                        <div class="flex flex-wrap gap-4">
                            ...
                            @if($user->github_url)
                                <a href="{{ $user->github_url }}" target="_blank"
                                    ...
                                    <svg class="w-5 h-5 group-hover:text-white" style="color: var(--text-primary);"
                                    ...
                                    <span
                                        class="absolute -bottom-8 left-1/2 -translate-x-1/2 px-3 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none text-xs glass-card"
                            ...
                            @if($user->linkedin_url)
                                <a href="{{ $user->linkedin_url }}" target="_blank"
                                    ...
                                    <svg class="w-5 h-5" style="color: var(--text-primary);" fill="currentColor"
                                        viewBox="0 0 24 24">
                                        <path
                            ...
                            @if($user->email)
                                <a href="mailto:{{ $user->email }}"
                                    ...
                                    <svg class="w-5 h-5" style="color: var(--text-primary);" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>
            ...
            <div class="max-w-7xl mx-auto px-6 md:px-12 py-6">
                <div class="flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
                    <p class="text-center md:text-left" style="color: var(--text-muted);">
                        Created with Detect ❤️ by {{ $user->full_name ?? 'Your Name' }}. © {{ date('Y') }} All Rights
                        Reserved.
                    </p>
                    </p>
